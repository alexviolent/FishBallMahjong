...
export class PinusWSClient {
    ...
    private onClose(e: any): void {
        ...
        this.emit(PinusWSClientEvent.EVENT_CLOSE,e);
    }

    private onIOError(e: any): void {
        this.emit(PinusWSClientEvent.EVENT_IO_ERROR, e);
        console.error('socket error: ', e);
    }

    private onKick(event: string) {
        this.emit(PinusWSClientEvent.EVENT_KICK,event);
    }
    ...
    private handshake(resData: any) {
        ...
        if (data.code === this.RES_OLD_CLIENT) {
            this.emit(PinusWSClientEvent.EVENT_IO_ERROR, 'client version not fullfill');
            return;
        }

        if (data.code !== this.RES_OK) {
            this.emit(PinusWSClientEvent.EVENT_IO_ERROR, 'handshake fail');
            return;
        }
